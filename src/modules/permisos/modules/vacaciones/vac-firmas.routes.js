...
router.post("/:id/confirmar-firma-jefe", requireAuth, requireFirmante, async (req, res) => {
  ...
  if (!password) return res.status(400).json({ message: "Contraseña del token requerida" });
  ...
  try {
    ...
    if (solicitud.estado !== "PENDIENTE_JEFE")
      return res.status(409).json({ message: "Esta solicitud no está pendiente de jefe" });
    ...
    const p12FullPath = path.resolve(process.env.UPLOADS_DIR || "uploads", p12Path.replace("/uploads/", ""));
    ...
    return res.json({ message: "Solicitud firmada y enviada al siguiente paso" });
  } catch (err) {
    return res.status(500).json({ message: "Error firmando solicitud", error: err.message });
  }
});
...
router.post("/:id/confirmar-firma-superior", requireAuth, requireFirmante, async (req, res) => {
  ...
  try {
    ...
    if (solicitud.jefe_firmante_id === firmante_id)
      return res.status(403).json({ message: "No puedes aprobar tu propia solicitud en dos pasos del flujo" });
    if (solicitud.gerente_id !== firmante_id)
      return res.status(403).json({ message: "No autorizado" });
    if (solicitud.estado !== "PENDIENTE_GERENTE")
      return res.status(409).json({ message: "Esta solicitud no está pendiente de jefe superior" });
    ...
    if (solicitud.archivo_jefe) {
      const jefeFilePath = path.resolve(process.env.UPLOADS_DIR || "uploads", solicitud.archivo_jefe.replace("/uploads/", ""));
...
router.post("/:id/confirmar-firma-uath", requireAuth, requireFirmante, async (req, res) => {
  ...
  try {
    ...
    // Tomar PDF del superior si existe, sino del jefe, sino generar base
    let pdfBuffer;
    ...
    await pool.query(`
      UPDATE core.vacacion_solicitud
      SET estado = 'APROBADO', fecha_respuesta_uath = NOW(), uath_id = $1, archivo_uath = $2
      WHERE id = $3
    `, [firmante_id, archivoPath, id]);
