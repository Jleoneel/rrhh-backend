...
const CARGOS_PERMITIDOS = [
  ...
  CARGO_NOMBRES.ANALISTA_TALENTO_HUMANO,
...
export async function createFirmanteUath(req, res) {
  ...
  if (!cedula || !nombre || !password || !cargo) {
    return res
      .status(400)
      .json({ message: "cedula, nombre, password y cargo son requeridos" });
  }
  ...
  try {
    ...
    if (existe.rowCount > 0) {
      return res
        .status(409)
        .json({ message: "Ya existe un firmante con esa cédula" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Firmante creado", firmante: rows[0] });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
}
...
export async function updateFirmante(req, res) {
  const { id } = req.params;
  const { nombre, activo, cargo } = req.body;

  if (nombre == null && activo == null && cargo == null) {
    return res
      .status(400)
      .json({ message: "Debe enviar nombre, activo y/o cargo" });
  }
  ...
  try {
    ...
    if (cargo != null) {
      ...
      cargoId = await getCargoId(cargo);
    }
